...
/* dYdA */
Eigen::Matrix<double,6,6> dso::state_partials(dso::OrbitalElements &ele, double GM,
  ...
  const double r = state.block<3,1>(0,0).norm();
      ...
      Eigen::Matrix3d PQW(
          ...
          Eigen::AngleAxisd(-ele.Omega(), Eigen::Vector3d::UnitX()));
  ...
  dYda << ( (x/a)*P + (y/a)*Q ) , ((-vx/(2e0*a))*P + (-vy/(2e0*a))*Q);
  Eigen::Matrix<double,6,1> dYde;
  dYde << ((-a-std::pow(y/fac,2e0)/r)*P + (x*y/(r*fac*fac))*Q) ,
    ((vx*(2e0*a*x+e*std::pow(y/fac,2e0))/(r*r))*P
    + ((n/fac)*std::pow(a/r,2e0)*(x*x/r-std::pow(y/fac,2e0)/a))*Q );
  Eigen::Matrix<double,6,1> dYdM;
  dYdM << ( (vx*P+vy*Q)/n ), ((-n*std::pow(a/r,3e0))*(x*P+y*Q) );
