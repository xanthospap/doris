...
int parse_rinex_rfos(const char *fnrnx, bool use_tai, int every, Datetime &tref,
                     const dso::PolynomialModel<Datetime> &fit,
                     Eigen::VectorXd &b, Eigen::MatrixXd &A, unsigned &idx,
                     const Datetime &start, const Datetime &end) noexcept {
...
int dso::fit_relative_frequency_offset(const std::vector<const char *> &fns,
                                       dso::PolynomialModel<Datetime> &fit,
                                       bool use_tai, int every,
  ...
  for (const auto sptr : fns) {
    if (parse_rinex_rfos(sptr, use_tai, every, tref, fit, b, A, index, start,
                         end)) {
      fprintf(stderr,
