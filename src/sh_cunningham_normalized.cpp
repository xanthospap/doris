...
int test::gravacc2(const dso::HarmonicCoeffs &cs,
  ...
  for (int m = degree; m >= 1; --m) {
    ...
    for (int n = degree; n >= m; --n) {
      ...
      acx +=
          F.f3(n) * (std::sqrt((double)arxy1) * (-cs.C(n, m) * M(n + 1, m + 1) -
                                                 cs.S(n, m) * W(n + 1, m + 1)) +
                     std::sqrt((double)arxy2) * (cs.C(n, m) * M(n + 1, m - 1) +
                                                 cs.S(n, m) * W(n + 1, m - 1)));
      acy +=
          F.f3(n) * (std::sqrt((double)arxy1) * (-cs.C(n, m) * W(n + 1, m + 1) +
                                                 cs.S(n, m) * M(n + 1, m + 1)) +
                     std::sqrt((double)arxy2) * (-cs.C(n, m) * W(n + 1, m - 1) +
                                                 cs.S(n, m) * M(n + 1, m - 1)));
