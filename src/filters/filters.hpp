...
struct EkfFilterImpl {
    ...
    static std::size_t num_params(int num_stations) noexcept {
        printf(">> asked for num. params, answer is %d (computed with %d stations)\n", 6+Np+num_stations+num_stations*((bcm==BeaconClockModel::None)?(1):(2)), num_stations);
        return 
...
template<int Np>
struct EkfFilter<Np, BeaconClockModel::Linear> {
...
#ifdef DEBUG
        assert(tropo_index(beacon_nr)>=0 && tropo_index(beacon_nr)<_ekf._ekf.x.rows() );
#endif
