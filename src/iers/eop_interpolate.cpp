...
int dso::EopLookUpTable::interpolate_lagrange(
    double tt_fmjd, dso::EopRecord &eopr, int order) const noexcept 
{
  int status = 0;
  int index = -1;
  status +=
...
int dso::EopLookUpTable::interpolate(double tt_fmjd,
                                     dso::EopRecord &eopr, int order) const noexcept {
