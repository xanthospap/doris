...
int dso::utils::celestrak::details::parse_csv_for_date(
  ...
  // note that we will also need data for the previous day
  dso::modified_julian_day mjd_prev(mjd.as_underlying_type() - 1);
  ...
  // keep on parsing/reading lines, untill we reach one prior to target date
  // or EOF
  int error = 0;
  while (cmjd != mjd_prev && !error) {
    if (fin.getline(line, MAX_SW_CHARS))
  ...
  // date matched! relevant record is stored in line (note that we are
  // talking about the 1-target_date !
  if (resolve_csv_line_records(line, flux_data[0])) {
    fprintf(stderr, "ERROR. Failed resolving flux data from line %s\n", line);
    return parse_error;
  }

  // we have the previous day line; read one more to reach the target date
  if (fin.getline(line, MAX_SW_CHARS)) {
    error = resolve_csv_line_date(line, cmjd);
  } else {
    return error;
  ...
  if (cmjd != target) {
    fprintf(stderr, "ERROR. Error parsing SW CVS file %s (%s)\n", fncsv,
            __func__);
    return parse_error;
  }
  if (resolve_csv_line_records(line, flux_data[1])) {
    fprintf(stderr, "ERROR. Failed resolving flux data from line %s\n", line);
    return parse_error;
  }

  // yet one more to reach the target+1_day date
  if (fin.getline(line, MAX_SW_CHARS)) {
    error = resolve_csv_line_date(line, cmjd);
  } else {
    return error;
  }
  if (cmjd != target + dso::modified_julian_day{1}) {
    fprintf(stderr, "ERROR. Error parsing SW CVS file %s (%s)\n", fncsv,
            __func__);
    return parse_error;
  }
  if (resolve_csv_line_records(line, flux_data[2])) {
    fprintf(stderr, "ERROR. Failed resolving flux data from line %s\n", line);
    return parse_error;
  }
