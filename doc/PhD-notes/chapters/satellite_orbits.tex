...
three additional parameters, called the \emph{orbital elements} or 
\emph{Euler angles}. The six orbital elements, are:
\begin{description}
    ...
    \item $\bm{\theta}$: \emph{true anomaly} (sometimes $\bm{M}$, the \emph{mean anomaly} 
    is used instead)
\end{description}
...
position along the orbit and the remnianing three elements, $\Omega$, $\omega$ 
and $i$ define the orientation of the orbit in space. Given these six elements, 
it is always possible to uniquely calculate the state vector.
