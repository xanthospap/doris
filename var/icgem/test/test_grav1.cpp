...
int main(int argc, char *argv[]) {
    ...
    int error = lagrange_polynomials(vec[0], vec[1], vec[2], Re, degree, order, V, W);
    if (error) {
    ...
    int error2 = lagrange_polynomials(vec[0], vec[1], vec[2], Re, degree, order, Vt, Wt);
    if (error2) {
    ...
    int error3 = lagrange_polynomials(vec[0], vec[1], vec[2], Re, degree, order3, Vt3, Wt3);
    if (error3) {
      fprintf(stderr, "ERROR Computing Lagrange polynomials! #2\n");
      return 1;
    }
