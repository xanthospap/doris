...
int dso::moon_vector(double tt_mjd, double *rsun) noexcept {
  const double t =
      (tt_mjd - /*MJD_J2000*/ 51544e0) / 36525e0; // Julian cent. since J2000
  ...
  #ifdef DEBUG
    printf("Fundamental Arguments for t=%15.5f (mjd=%15.5f)\n", t, tt_mjd);                                            
    printf("L0 = %15.10e\n", L0);                                                
    printf("l  = %15.10e\n", l);                                                  
    printf("lp = %15.10e\n", lp);                                                 
    printf("D  = %15.10e\n", D);                                                  
  ...
  // lunar longitude
  const double lambda =
      L0 + c[0] * sin(l) + c[1] * sin(2e0 * l) +
      c[2] * sin(l - 2e0 * D) + c[3] * sin(2e0 * D) +
      c[4] * sin(lp) - c[5] * sin(2e0 * F) +
      c[6] * sin(2e0 * l - 2e0 * D) - c[7] * sin(l + lp - 2e0 * D) +
      c[8] * sin(l + 2e0 * D) - c[9] * sin(lp - 2e0 * D) +
      c[10] * sin(l - lp) - c[11] * sin(D) +
      c[12] * sin(l + lp) - c[13] * sin(2e0 * F - 2e0 * D);

  // lunar latitude
  const double beta =
      18520e0 * sin(F + lambda - L0 + 412e0 * sin(2e0 * F) +
                         541e0 * sin(lp)) -
  ...
  // distance
  const double r =
      385000e0 - 20905e0 * cos(l) - 3699e0 * cos(2e0 * D - l) -
