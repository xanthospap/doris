...
int main(int argc, char* argv[])
{
  ...
  // index of the C1 measurement (pressure)
  int c1_idx = rnx.get_observation_code_index(
      ids::ObservationCode { ids::ObservationType::pseudorange, 1 });
  if (c1_idx < 0) {
    fprintf(stderr,
        "[ERROR] Failed to find ObservationType C1 in RINEX\'s observation "
        "types vector! (traceback: %s)\n",
        __func__);
    return 1;
  }
  
  // index of the 21 measurement (pressure)
  int c2_idx = rnx.get_observation_code_index(
      ids::ObservationCode { ids::ObservationType::pseudorange, 2 });
  if (c2_idx < 0) {
    fprintf(stderr,
        "[ERROR] Failed to find ObservationType C2 in RINEX\'s observation "
        "types vector! (traceback: %s)\n",
        __func__);
    return 1;
  }
  
  // index of the W1 measurement (pressure)
  int w1_idx = rnx.get_observation_code_index(
      ids::ObservationCode { ids::ObservationType::power_level, 1 });
  if (w1_idx < 0) {
    fprintf(stderr,
        "[ERROR] Failed to find ObservationType W1 in RINEX\'s observation "
        "types vector! (traceback: %s)\n",
        __func__);
    return 1;
  }
  ...
  // index of the 21 measurement (pressure)
  int w2_idx = rnx.get_observation_code_index(
      ids::ObservationCode { ids::ObservationType::power_level, 2 });
  if (w2_idx < 0) {
    fprintf(stderr,
        "[ERROR] Failed to find ObservationType W2 in RINEX\'s observation "
        "types vector! (traceback: %s)\n",
        __func__);
    return 1;
  }
  ...
  // index of the 21 measurement (pressure)
  int t_idx = rnx.get_observation_code_index(
      ids::ObservationCode { ids::ObservationType::ground_temperature});
  if (t_idx < 0) {
    fprintf(stderr,
        "[ERROR] Failed to find ObservationType T in RINEX\'s observation "
        "types vector! (traceback: %s)\n",
        __func__);
    return 1;
  }
  
  int h_idx = rnx.get_observation_code_index(
      ids::ObservationCode { ids::ObservationType::ground_humidity});
  if (h_idx < 0) {
    fprintf(stderr,
        "[ERROR] Failed to find ObservationType H in RINEX\'s observation "
        "types vector! (traceback: %s)\n",
        __func__);
    return 1;
  }
  ...
  while (!(error = it.next())) {
    ...
    while (beaconobs_set != it.cblock.end()) {
      ...
      if (beac_it == prv_data.end()) { // no previous obs; just update the array
        ...
        double l1_raw = beaconobs_set->m_values[l1_idx].m_value;
        double l2_raw = beaconobs_set->m_values[l2_idx].m_value;
        double c1_raw = beaconobs_set->m_values[c1_idx].m_value;
        double c2_raw = beaconobs_set->m_values[c2_idx].m_value;
        double w1_raw = beaconobs_set->m_values[w1_idx].m_value;
        double w2_raw = beaconobs_set->m_values[w2_idx].m_value;
        double p_raw = beaconobs_set->m_values[p_idx].m_value;
        double t_raw = beaconobs_set->m_values[t_idx].m_value;
        double h_raw = beaconobs_set->m_values[h_idx].m_value;
        double f_raw = beaconobs_set->m_values[f_idx].m_value;

        printf("%.3s %.8f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f\n", beacon_internal_id, tnow.as_mjd(), l1_raw, l2_raw, c1_raw, c2_raw, w1_raw, w2_raw, p_raw, t_raw, h_raw, f_raw);
