...
int apply_iono_correction = 0;
int new_arc_on_discontinuity = 0;
...
int main(int argc, char* argv[])
{
  ...
  // index of the 2GHz code measurement
  ...
  // index of the 400MHz code measurement
  ...
  // index of the 400MHz phase measurement
  ...
  for (int i = 0; i < (int)rnx.stations().size(); i++) {
    Eigen::Matrix<double,3,1> bcrd; bcrd << crd[i].x, crd[i].y, crd[i].z;
    const auto ell = dso::car2ell<dso::ellipsoid::grs80>(bcrd);
    crd_ell[i].x = ell(0);
    crd_ell[i].y = ell(1);
    crd_ell[i].z = ell(2);
  }
  ...
  printf("ID MJD  Nominal frequencies NDoppler dt(sec) Df/f0 r-r0/dt F1 F2\n");
  ...
  while (!(error = it.next())) {
    ...
    while (beaconobs_set != it.cblock.end()) {
      ...
      if (beac_it == prv_data.end()) { // no previous obs; just update the array
        ...
        printf("%.3s %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f // %.5f %.5f %.5f %.5f [%c%c]\n",
            beacon_internal_id, 
            tnow.as_mjd(), 
            beaconobs_set->m_values[l1_idx].m_value,
            beaconobs_set->m_values[l2_idx].m_value,
            beaconobs_set->m_values[c1_idx].m_value,
            beaconobs_set->m_values[c2_idx].m_value,
            beaconobs_set->m_values[f_idx].m_value,
            beaconobs_set->m_values[p_idx].m_value,
            nf2g, 
            nf4m, 
            Ndop, 
            Dtsec, 
