    ...
    if line[0] != '#':
        ...
        try:
            if line.startswith('[MOON]'):
                dct_moon.append({'mjd': handle_date(float(l[1])), 'rax':float(l[2]), 'ray':float(l[3]), 
                    'raz':float(l[4]), 'ax':float(l[5]), 'ay':float(l[6]), 'az':float(l[7])})
            if line.startswith('[SUN]'):
                dct_sun.append({'mjd': handle_date(float(l[1])), 'rax':float(l[2]), 'ray':float(l[3]), 
                    'raz':float(l[4]), 'ax':float(l[5]), 'ay':float(l[6]), 'az':float(l[7])})
        except:
...
for planet in ['Moon', 'Sun']:
    if planet == 'Moon': dct = dct_moon
    if planet == 'Sun': dct = dct_sun
    ...
    fig.suptitle('COST-G Benchmark Diffs\n{:} Acceleration Comparisson'.format(planet), fontsize=16)
    plt.savefig('03directTide{:}_icrf.pdf'.format(planet))
    plt.show()
