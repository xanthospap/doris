...
int main(int argc, char *argv[]) {
  ...
  if (argc != 5) {
    fprintf(stderr,
            ...
            "[03directTideSun_icrf.txt] [spk kernel, de421] [lsk kernel, naif0012.tls]\n",
            argv[0]);
    return 1;
  }
  ...
  for (const auto &crf : icrf) {
    ...
    sun_acc =
        dso::point_mass_accel(1.32712442076e20, crf.xyz, rSun * 1e3, partials);
    ...
    if (ait != accs.cend()) {
      printf("%.12f %.5e %.5e %.5e\n", crf.mjd.mjd(), ait->a(0)-sun_acc(0),ait->a(1)-sun_acc(1),ait->a(2)-sun_acc(2));
