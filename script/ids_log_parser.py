...
def parse_first_log_line(line):
  ...
  if not m or len(m.groups()) != 1:
    errmsg = 'ERROR. Failed to match expected pattern in fist line \'{:}\''.format(line.strip)
...
class IdsLogFile:
    ...
    while line:
      if len(line):
        ...
        if re.match(r"{:}.[0-9]+\s*".format(block_nr), line):
          ...
          if len(l) != 2:
            errmsg = "ERROR. Failed to parse line \'{:}\'".format(line.strip())
            raise RuntimeError(errmsg)
          cdic[l[0]] = l[1]
          next_line_read = True
          line = self.istream_.readline()
        ...
        if not next_line_read: line = self.istream_.readline()
      else:
        line = self.istream_.readline()
  ...
  def parse_block_nr(self,block_nr):
    if not self.go_to_block(block_nr):
      errmsg = "ERROR. Failed to find block nr {:} in file: ".format(block_nr, self.filename_)
      raise RuntimeError(errmsg)
    return self.parse_this_block(block_nr)

def dump_log_antenna_info(logfn):
  ...
  for subblock in lofd:
    ...
    if len(m.groups()) !=1 :
      errmmsg = "ERROR. Failed to parse site height from field: [{:}]".format(height_str)
      raise RuntimeError(errmsg)
    height = float(m.group(1)) if m.group(1) else 0e0
    installed_at = datetime.datetime.strptime(subblock["Date installed (dd/mm/yy)"], "%d/%m/%Y")
    removed_at =  datetime.datetime.strptime(subblock["Date removed (dd/mm/yy)"], "%d/%m/%Y") if subblock["Date removed (dd/mm/yy)"] != "" else datetime.datetime.max
    print("{:4s} {:} {:} {:8.6f}".format(name,installed_at.strftime("%Y-%m-%d %H:%M:%S"),removed_at.strftime("%Y-%m-%d %H:%M:%S"),height))
